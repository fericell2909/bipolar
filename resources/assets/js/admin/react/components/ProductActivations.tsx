...
export default class BipolarProductActivations extends Component<any, any> {
  state = {
    actives: [],
    activesFiltered: [],
    inactives: [],
  ...
  toggleActive(productHashId, active) {
    ...
    axios
      .put(`/ajax-admin/products/${productHashId}`, { active: !active })
      .then(() => {
    ...
    const activesRendered = activesForFilter.map(activeItem => {
      ...
      return (
        <tr key={activeItem['hash_id']}>
          <td>{activeItem['id']}</td>
          <td>
            <img src={firstPhotoUrl} width="100" />
          </td>
          <td>{activeItem['name']}</td>
          <td>{activeItem['price']}</td>
          <td>
            <span className="label label-pill label-success">Activo</span>
          </td>
          <td className="text-center">
            <button
              onClick={() => this.toggleActive(activeItem['hash_id'], activeItem['active'])}
    ...
    const inactivesRendered = inactivesForFilter.map(inactiveItem => {
      ...
      return (
        <tr key={inactiveItem['hash_id']}>
          ...
          <td>
            <span className="label label-pill label-danger">Inactivo</span>
          </td>
          <td className="text-center">
            <button
              onClick={() => this.toggleActive(inactiveItem['hash_id'], inactiveItem['active'])}
              className="btn btn-dark btn-rounded"
              disabled={this.state.waitingAjax}>
        ...
        <div className="col-md-6">
          ...
          <div className="white-box">
            ...
            <table className="table table-responsive">
              ...
              <tbody>{activesRendered.length > 0 ? activesRendered : null}</tbody>
            </table>
          </div>
        </div>
        <div className="col-md-6">
          <div className="white-box">
            <div className="input-group">
              <input
                type="text"
                className="form-control"
                placeholder="Buscar en inactivos"
                onKeyUp={this.searchInactives}
              />
            </div>
          </div>
          <div className="white-box">
            ...
            <table className="table table-responsive">
              ...
              <tbody>{inactivesRendered.length > 0 ? inactivesRendered : null}</tbody>
            </table>
          </div>
        </div>
      </div>
  ...
  getActivesAndInactives() {
    ...
    return axios
      ...
      .then(
        axios.spread((responseActive, responseInactive) => {
          this.setState({
