...
class MultipleDiscountsEdit extends React.Component<any> {
  state = {
  ...
  handleSaveDiscount = event => {
    ...
    axios
      .put(`/ajax-admin/discount-tasks/${this.props.taskId}`, {
  ...
  getData = async () => {
    ...
    const selectedSubtypes = subtypes
      .filter(subtype => task['product_subtypes'].includes(subtype['id']))
      .map(this.mapSubtypes);
    const selectedTypes = types
      .filter(subtype => task['product_types'].includes(subtype['id']))
      .map(this.mapTypes);
    const selectedProducts = products
      .filter(product => task['products'].includes(product['id']))
  ...
  render() {
    const optionProducts = this.state.productsCopy.length
      ? this.state.productsCopy.map(this.mapProducts)
      : [];
    const optionSubtypes = this.state.subtypes.length
      ? this.state.subtypes.map(this.mapSubtypes)
      : [];
...
if (document.getElementById('bipolar-product-multiple-discounts-edit')) {
  const BipolarTaskId = (window as any).BipolarDiscountTaskId;
  ReactDOM.render(
    <MultipleDiscountsEdit taskId={BipolarTaskId} />,
    document.getElementById('bipolar-product-multiple-discounts-edit')
  );
}
