...
export default class BipolarProductNew extends React.Component {
        ...
        const colorsRender = colors.map(color => {
            ...
            return (
                <div key={color['hash_id']} className="checkbox">
                    <label><input type="checkbox" checked={isSelected} value={color['hash_id']} onChange={this.handleColorChange}/>{color['name']}</label>
                </div>
            );
        });
        ...
        const sizesRender = this.state.sizes.map(size => {
            ...
            return (
                <div key={size['hash_id']} className="checkbox">
                    <label><input type="checkbox" checked={isSelected} value={size['hash_id']} onChange={this.handleSizeChange}/>{size['name']}</label>
                </div>
            );
        });
                ...
                subtypes = type['subtypes'].map(subtype => {
                    ...
                    return (
                        <div key={subtype['hash_id']} className="checkbox">
                            <label><input type="checkbox" checked={isSelected} value={subtype['hash_id']} onChange={this.handleSubtypeChange} />{subtype['name']}</label>
                        </div>
                    );
                });
            }
            ...
            <div className="row">
                        ...
                        <div className="form-row">
                            <div className="col-md-6">
                                <div className="form-group">
                                    <label>Nombre</label>
                                    <input value={this.state.name} onChange={this.handleInputChange} name="name" type="text" className="form-control" required/>
                                </div>
                            </div>
                            <div className="col-md-6">
                                <div className="form-group">
                                    <label>Precio</label>
                                    <input value={this.state.price} onChange={this.handleInputChange} name="price" type="number" className="form-control"/>
                                </div>
                            </div>
                        </div>
                        <div className="form-group">
                            <label>Descripción (Opcional)</label>
                            <textarea value={this.state.description} onChange={this.handleInputChange} name="description" className="form-control" rows="7"/>
                        </div>
                        <div className="row">
                            <div className="col-md-5">
                                <label className="checkbox-inline" checked={this.state.salient} onChange={this.handleSalientChange}><input type="checkbox"/>Destacado</label>
                            </div>
                        </div>
                        <hr/>
                ...
                <div className="col-md-3">
                    <div className="white-box">
                        ...
                        <div className="panel-wrapper collapse in">
                            <div className="panel-body">
                                <input value={this.state.textSearchColors} onChange={this.handleSearchColors} type="text" className="form-control" placeholder="Buscar color"/>
    ...
    getAllInformation() {
        axios.all([
            ...
            .then(axios.spread((responseColors, responseSizes, responseTypes) => {
                this.setState({
